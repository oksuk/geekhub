...
(function ($) {
  ...
  Drupal.wysiwyg.plugins['gmf_tokenbuilder'] = {
    ...
    attach: function (content, settings, instanceId) {
      ...
      if (matches) {
        for (i = 0; i < matches.length; i++) {
          var inlineTag = matches[i].replace('[gmf:', '').replace(']', '');
          var toInsert = this._getPlaceholder(settings, inlineTag);
          content = content.replace('[gmf:'+inlineTag+']', toInsert);
        }
      }
      return content;
    },
    ...
    detach: function (content, settings, instanceId) {
      ...
      if (matches = content.match(/<img[^>]+class=([\'"])google-map-field-tokenbuilder-pin[^>]*>/gi)) {
        for (var i = 0; i < matches.length; i++) {
          ...
          var token = '';
          token += 'lat='+$(imgNode).attr('lat');
          token += ',lon='+$(imgNode).attr('lon');
          token += ',width='+$(imgNode).attr('map_width');
          token += ',height='+$(imgNode).attr('map_height');
          token += ',zoom='+$(imgNode).attr('map_zoom')+';';
          //var token = $(imgNode).attr('gmfdata');
    ...
    add_form: function (data, settings, instanceId) {
      ...
      dialogdiv.load(aurl + " .content #google-map-field-tokenbuilder-form", function(){
        ...
        if (data.mapSettings == null) {
          ...
          google_map_field_getMap(data.mapSettings.lat, data.mapSettings.lon, parseInt(data.mapSettings.zoom));
    ...
    insertIntoEditor: function (settings, token, editor_id) {
      tag = "<img class='google-map-field-tokenbuilder-pin' src='"+settings.path+"/images/gmftb.toolbar_icon.png' "+token+" title='Google Map'>";
    ...
    _getPlaceholder: function (settings, token) {
      ...
      return "<img class='google-map-field-tokenbuilder-pin' src='"+settings.path+"/images/gmftb.toolbar_icon.png' "+attrs+" title='Google Map'>";
    },
    ...
    _getMapSettings: function (data) {
      var mapSettings = {};
      mapSettings.lat = $(data).attr('lat');
      mapSettings.lon = $(data).attr('lon');
      mapSettings.width = $(data).attr('map_width');
      mapSettings.height = $(data).attr('map_height');
      mapSettings.zoom = $(data).attr('map_zoom');
      return mapSettings;
    },
    ...
    _getAttrs: function (data) {
      var mapSettings = {};
      token = data.replace('[gmf:', '').replace(']', '');
      token = data.replace(';', '');
      ...
      var height = mapSettings.height;
      return "lat='"+lat+"' lon='"+lon+"' map_width='"+width+"' map_height='"+height+"' map_zoom='"+zoom+"'";
    }
